...
- **Repair-Retest V1 Rule Spec（2026-08-31 冻结，A+ 版本化 Rule Spec）**：
  ...
  - **HTML 报告（8 列 Human Decision View）**：`current_eval_report.py` 是**纯 renderer**——只消费 current_watch_eval.json 已有 stage/evidence/odds/history，绝不重算（测试锁定）。8 列 = code/name/RR阶段/median_120d/胜率/payoff/时间证据/最终判断；按 `odds_assessment` 排序（strong_observe→watch_structure→position_only→cautious→out_of_domain_good→bad→unreliable，组内 median_120d DESC 缺失最后 + fund_code ASC）；`YEAR_DEPENDENT`/TARGET 提供轻量可折叠 by_year 详情（年份/n/median/win）审计。产物 `current_odds_report.html`
  - **集合定义锁定**：`in_domain = stage∈{TARGET,IN_DOMAIN_NON_TARGET}` / `cross_year = evidence_label==CROSS_YEAR_SUPPORTED` / `focus = odds∈{strong_observe,watch_structure}` / `unreliable = stage==UNRELIABLE`——展示层不得改口径
