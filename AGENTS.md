...
- **四段选筹（v0.9.0，个股保留）**：Layer③ **个股**走统一四段——**① trend**（趋势门）→ **② leadership**（score_trend 主题内排名 → LEADER/CORE）→ **③ position**（历史位置/赔率；`config/strategy_spec.yaml historical_position`，默认 `metric=ma60_deviation` 乖离率）→ **④ signal**（LEADER×LOW→STRONG_BUY、LEADER×MID→BUY、CORE×LOW→BUY、CORE×MID→WATCH、HIGH→HOLD、BREAKDOWN→WAIT、fallback WAIT）。**ETF 路径自 v0.12.0 起不再走「最强选筹」，改走 Eligibility/Vehicle/Timing 三段（见下）。**
- **Layer③ ETF Selection V2（v0.12.0，Eligibility / Vehicle / Timing）**：ETF 表达路径从「主题里找最强 → 看能不能买」重写为三段（`selection.py`）：
  - **③A Eligibility**：`_etf_eligible_pool`——哪些 ETF 可靠可用进入车辆宇宙（theme 归属 + 账户内可交易 + 流动性≥min_amount + **lane2_reliable_360 is not False**（lane-less 放行）+ rps15 有效）。**趋势态不作资格**（vehicle 身份不随当日涨跌而变；`ETF_TREND_GATES` 只作 ③C 展示口径）。数据可靠性前置到资格层 → 不可靠高分 ETF 不再抢方向 dedup 席位后被后置 veto。
  - **③B Vehicle Selection**：`_dedup_etf`——同方向（方向词）在 **eligible 宇宙内**按 vehicle_score 取代表；`selection_score` 语义收敛为**车辆适配度**（`strategy_spec etf_selection.vehicle.weights`，默认 amount 权重；**rps15/20 不进选车**——时机波动不换车）。core=CORE_ETF(rank1 LEADER)、sub=SUB_INDUSTRY_ETF(rank2..3 CORE)。
  - **③C Timing**：对选定车辆独立判技术时机（leadership→position→signal），`recommended = theme_confirmed ∧ trend_gate ∧ signal∈{BUY,STRONG_BUY}`（显式 AND theme_confirmed，修「未确认主题仍 recommended=True」缺陷）；Lane2/3 降为 soft context 透传（可靠性已进 ③A，不再后置否决）。
  - `eligible_etf_count` = ③A 通过数（产品门/OBSERVE vs WAIT_FOR_ETF 分界），不再含趋势门。观察池 `etf_pool`/`theme_etf_pool` 全量展示 + reason_codes（vehicle_eligible / lane2_unreliable / below_trend_gate(③C) / low_liquidity / dedup_lost(仅 eligible 内)）。`RULE_VERSION`=v0.12.0。
...
- **ETF-only 发布 + 三 Lane Validation（v0.11，2026-09）**：Layer③ 每日发布输出**收敛 ETF-only**——`select run` 固定 `build_candidates(include_stocks=False)`（输出 JSON 标 `meta.scope=etf_only`，报告无个股区块）；**个股仍在 Layer② Tier 作行业确认输入，只是不再进 Layer③ 发布候选**。engine 默认 `include_stocks=True` 供 research replay/parity 重放历史个股（Engine 与发布层分离）。消费 `three_lane_{trade_date}.parquet`（ETF State Fusion 单一 join，exact 对齐；缺失 → lane-less，`lane_trade_date=None/lane_lag_days=None`，禁 fallback 前后日期），ETF 候选/观察透传 `lane2_reliable_360/lane2_long_term_bottom/lane2_target_stage/lane2_bottom_state/lane3_transition_state/lane3_days_since_first_exit`。**Validation 语义（锁定）**：L2 数据可靠性=**硬 gate**（`strategy_spec etf_selection.lane_validation.reliability_hard_gate_enabled`；three_lane 明确 `lane2_reliable_360=False` → 本可推荐 ETF 强制不可推荐，`reason=lane2_unreliable`；lane-less 不触发）；L2 结构（repair-retest TARGET）=soft 仅标注；**L3 阶段=纯 context 不 gate**（3B FAIL 结论）。`RULE_VERSION`=v0.11.0 → **v0.11.1（2026-09，Theme Mapping）** → **v0.12.0 起 L2 可靠性 gate 语义前移**：不再「先推荐后 veto」，改在 ③A Eligibility 把 `lane2_reliable_360=False` 剔除出车辆宇宙（见「Layer③ ETF Selection V2」）
- **策略语义分层**：`strategies.{ai_20,ai_ma,hc_20}` 回答「入场后持有多久/怎么退出」（回测/组合层），与四段选筹「选哪个标的」是两件事，互不重叠
- **Theme Mapping（v0.11.1）**：`theme_registry.yaml` 每 theme 增加可选 `etf_exclude_keywords`（theme 级反向排除子串，如 AI 基建 = 港股通/恒生/中概/纳斯达克/标普），`match_theme` 先 exclude 后 include（`src/common/themes.py`）。背景：20260902「港股通信息技术ETF」因名称含子串「通信」被 AI 基建关键词误吸收并获推荐（159196）——只删泛关键词无效，必须显式 exclude。同步从 AI 基建 `etf_keywords` 删除泛词（信息技术/电子/计算机/大数据，保留云计算等真基建词）；港股通信息技术/恒生/中概 ETF 归 None。`RULE_VERSION`=v0.11.1。replay/parity 仅比重叠 key，关键字裁剪只影响主题归属（config_hash 变属预期）。
- ETF 候选（v0.12.0 起）动态从 Layer① rotation 全市场按 `theme_registry.yaml` 主题关键词选，走 **③A Eligibility → ③B Vehicle（方向代表，vehicle_score=amount）→ ③C Timing**；`etf_pool` 为全部关键词命中观察池（reason_codes：vehicle_eligible/lane2_unreliable/below_trend_gate/low_liquidity/dedup_lost，供「⑤ 观察」展示未入选原因）
...
- **报告声明式 Spec（v2，v0.12.1 Selection V2 IA）**：HTML 由 Report Engine 解释 `src/selection/report_spec.yaml` + ReportViewModel 生成，report.py 只是薄入口。**决策行为真源 = selection/recommendation；报告结构真源 = report_spec.yaml；人类可读说明 = docs/SELECTION_REPORT.md**。Spec 管结构/顺序/列/标签/条件/优先级；派生字段（classification/panel/change/why/next_trigger）全部在 ReportViewModel，绝不写回核心 JSON。**阅读顺序按决策漏斗**：主题成立 → ③A 可靠可用 ETF → ③B 表达载体 → ③C 今日执行 → Why now → Next trigger → 决策审计。**③B 载体 ≠ ③C 可买**（载体入选但时机 WAIT 是常态，非「无合格标的」）——载体行永远展示选定 CORE/SUB，执行行单独给 BUY/WATCH/WAIT + Lane 原因
- **8 段 IA（v0.12.1）**：01 今日结论（action + one_liner + 可执行 chips）→ 02 Theme Confirmation（主题成立吗：确认面/依据/Tier）→ 03 ③A Eligibility（可靠可用 ETF：eligible_N / pool_N）→ 04 ③B 表达载体（选定 CORE/SUB + 载体适配度）→ 05 ③C Timing（载体当前执行：Lane1 趋势/Lane2/Lane3 → BUY/WATCH/WAIT）→ 06 Why Now（当前为什么是这结果）→ 07 Next Trigger（什么变化会改变 Policy 结果，只列真能改结果的条件）→ 08 决策审计（全 ETF · reason_codes · Lane 上下文，appendix）。跨日变化归 01（今日关键变化），不再单独成段。fixture：`tests/fixtures/report/selection_20260902.{json,golden.html}` 为 **canonical**（Selection V2），20260827 保留为 V1 legacy 对照
...
- Layer ③ 交易候选：`outputs/selection/tradable_candidates_{date}.json`（结构化推荐对象，`role: recommendation`，`layer3.buckets[].themes[]` —— 每主题 today/why/recommendation/rationale/watchlist 已降级为**底层数据 contract**，报告不再镜像）+ `.html`（标题「③ 今日投资建议」，**v0.12.1 决策漏斗 8 段**：01 今日结论 → 02 Theme Confirmation → 03 ③A Eligibility → 04 ③B 表达载体 → 05 ③C Timing → 06 Why Now → 07 Next Trigger → 08 决策审计；**③B 载体 ≠ ③C 可买**）。**报告结构真源 = `src/selection/report_spec.yaml`（声明式 Spec），HTML 由 Report Engine 解释 Spec + ReportViewModel 生成**；改报告内容改 Spec，不拼 HTML。逐模块/逐指标约定见 `docs/SELECTION_REPORT.md`（Spec 的人类镜像，YAML 为准）。跨日对比归 01（今日关键变化），读上一份 Layer③ JSON（按 date 取最近、fail-soft、版本不兼容跳过）；派生字段（classification/panel/why/next_trigger/display_state）全部在 ReportViewModel，绝不写回核心 JSON
...
- **Repair-Retest V1 Rule Spec（2026-08-31 冻结，A+ 版本化 Rule Spec）**：
  ...
  - **out_of_domain 三分类（2026-09-01）**：good=历史正收益·当前不在域；bad=历史负收益；**unknown=历史不足（INSUFFICIENT，不能判好坏）**——避免「历史不足」误读为「无吸引力」；**INSUFFICIENT_HISTORY 优先于 pooled median**（即使 median>0 也判 unknown，159666 n=1 即如此，与 159301 n=0 统一）
  - **产物**：`current_watch_eval.json` + `current_odds_table.csv` + `current_odds_report.html`
- **Full-Market V1 Daily Scanner（2026-09-01，Lane 2 日常入口）**：模块 `src/research/etf_bottom/scanner.py` + `scanner_report.py`（`research etf-bottom --scan [--date YYYY-MM-DD]` / `make etf-bottom-scan [SCAN_DATE=...]`；已挂入 `make run-day` 末端，只读 raw 缓存不联网）——**V1 Freeze → Application 的正式落地**：对全市场「通过数据质量门」的 reliable ETF 应用冻结 REPAIR_RETEST_V1，874 只今天的数据只进规则、永不反向重算 cut points
  - **Universe 数据驱动**：RELIABLE = full_360_sample ∧ ¬unreliable_360 ∧ **¬flat_price_noise**（2026-08-31 实测 874，货币/债券近零波动单列 audit 不参与底部判定）；**cohort 不硬编码**——BASE = reliable ∧ hist≥756（实测 664，V1 原始研究支持范围）、EXTENSION = reliable ∧ 360≤hist<756（实测 210，规则外推）；≥756D 但 360D 折算污染的 40 只不进任何 cohort（domain guard → UNRELIABLE）。**flat_price 与 unreliable 互斥两桶 audit**（货币/债券 27 只 + 数据门 388 只，不重复计数）。cohort 只是报告标记，不改变规则
  - **运行时防漂移守卫**：`_verify_frozen` 每次 run 校验 cut points 与 frozen YAML 一致，漂移抛错（改规则必须新建 V2）
  - **三层输出（各答一个不同的问题，不混层）**：**Layer A Market Bottom Map**（breadth：reliable/ltb/DEEP/RECOVERING/TARGET/NEAR_MISS 计数 + **industry_cluster 列直接输出产业簇集中度**（汽车/周期/军工航空/消费文旅/游戏传媒，renderer 不再从名字猜）+ 前一交易日迁移聚合）· **Layer B Repair-Retest Scanner**（域内全量：TARGET=pos60 Q1×pos120 Q3 / **NEAR_MISS=Q1×Q2 或 Q2×Q3，带 near_miss_reason（P120_ONE_BUCKET_AWAY / P60_ONE_BUCKET_AWAY）** / DEEP↔RECOVERING 迁移 / domain 进出，machine 表含 industry_cluster 列）· **Layer C Historical Odds**（只对 in-domain ∪ near-miss ∪ watch 池附 `attach_history`+`odds_assessment`，避免全市场全跑）
  - **transition 语义（统一 prev-trade-date，用户锁定）**：prev_trade_date 一律 = **统一的 market prev-trade-date**（as_of 前一交易日，全市场 raw 推导），**不是各 ETF 自己最近可用交易日**——两者不得混用。某 ETF 在统一 prev 日无数据（停牌/缺失）→ `prev_data_status=missing`、`transition=PREV_MISSING`、保留 `prev_actual_trade_date` 审计（如 560650 核心50ETF民生加银：prev=08-28 但自身最近 08-21 → PREV_MISSING）；正常行 `prev_data_status=ok`。其余 `DEEP_TO_RECOVERING / RECOVERING_TO_DEEP / ENTER_DOMAIN / EXIT_DOMAIN / STAY_IN_DOMAIN / OUTSIDE_DOMAIN`
  - **字段语义分离（用户锁定，三个概念不混）**：`long_term_bottom` = 市场状态事实（Observation，price_map 口径 pos120/360≤20）· `in_domain` = V1 research domain（= RELIABLE ∧ long_term_bottom）· `target_stage` = 应用层分类（TARGET/NEAR_MISS/NON_TARGET）。reliable 表内 long_term_bottom 与 in_domain 数值一致是**设计使然**（in_domain 定义含 reliable），但来源不同不得互相替换（测试锁定）
  - **Layer C 历史赔率字段契约（2026-09-01 修复）**：`attach_history` 返回 `{"history": {...}}` 嵌套结构，scanner 存 `odds` 时必须解嵌套存**内层 stats dict**（`attach_history(code)["history"]`），renderer 直接读 `r["odds"].get("median_120d")` 等——否则 n/120D 中位/胜率/Payoff 全空、`evidence_label` 全错成 INSUFFICIENT_HISTORY、`odds_assessment` 塌缩（实测修复前 22 只 in-domain 全标 position_only、159819 错标 out_of_domain_unknown）。测试锁定：517770 n=9 / +41.2% 必须出现在 Layer C 表格，159819 必须 out_of_domain_good
  - **产物**：`scan_{date}.json`（三层完整 + watch_pool_total + flat_price_audit + flat_parquet_path）+ `scan_{date}.parquet/csv`（全 reliable + cohort/stage/industry_cluster/near_miss_reason/transition/prev_data_status/prev_actual_trade_date 扁平列）+ `scan_{date}.html`（三层报告）；date-stamped，不写 `_latest`
  - **漏斗定位（2026-09-01）**：展示层 TARGET 中文名 = **「结构触发候选」**（机器码 `target_stage=TARGET` 保留不变）——「市场今天主动告诉我谁出现了结构」，与 **18 只观察池**（`watch_pool_total` 由 scanner 计算进 payload，renderer 不硬编码也不读 config）区分：观察池是独立的人工关注名单（我主动盯谁），**不是漏斗上一级**。漏斗 = 全市场 ETF → reliable → 长期底部域 → 结构触发候选(TARGET) → Current Odds → 最终判断；报告顶部 summary 渲染此说明。
  - **taxonomy 修正（2026-09-01）**：`calibrate_etf_type` 加「现金流」排除——现金流ETF（自由现金流/现金流指数）是权益资产，`_MONEY_KEYWORDS` 的「现金」子串会误命中「现金流」→ 被误判 money → 误排除出底部研究（**全市场 31 只名字含「现金流」的 ETF 全部被误判 money，修复前全部走 flat_price_noise 排除**）。修正后现金流ETF 归 theme、正常参与底部判定（159201/159399 回到 EXTENSION，reliable 由 901−27=874，现金流ETF 原本就在 901 内不构成额外加入）。货币/债券判定仍靠 flat-price guardrail 兜底
  - **current_eval AS_OF 已参数化**（默认仍 2026-08-28 兼容研究快照与测试），scanner 每日传最新交易日
  - **测试**：cohort 数据驱动逻辑、TARGET/NEAR_MISS frozen-cut 分类（含 near_miss_reason）、transition 语义（含 PREV_MISSING + prev_actual_trade_date 审计）、2026-08-31 基线（874/664/210/27/22→6+16/0 TARGET）、renderer 只消费 JSON、`_verify_frozen` 漂移守卫、in_domain=reliable∧ltb 语义契约、现金流ETF≠money
  - **语义收敛（替代旧口径）**：874 只不再是「可做底部研究的集合」，而是「2026-08-31 时点通过数据质量门、具备 V1 Application 资格的 ETF；664 属长历史 BASE、210 属较短历史 EXTENSION；另 27 只货币/债券 flat_price 单列 audit」——更准确，适合未来 OOS ledger

## Lane 3 Research（Trend Transition，独立于每日 pipeline）

- **定位**：三条 Lane 分工——Lane 1 Leadership（谁已经是强者，趋势延续）· Lane 2 Bottom/Repair（哪里有底部赔率，均值回归）· **Lane 3 Trend Transition（谁正在从弱势切换成趋势，状态转换）**。Lane 3 研究问题：一个资产从长期弱势/底部第一次向上切换（first exit）时，能否在当时识别这次切换会继续演化成趋势（escape），而不是重新退回底部（retest）。只做独立研究，**不进入 Layer ①/②/③ 生产决策链**。
- **Study 3A · Post-924 Bottom-to-Trend Transition（2026-09-01）**：模块 `src/research/trend_transition/`（`research trend-transition study3a` / `make trend-transition-3a`）——验证「底部 → 非底部」切换结构（transition phenomenon）在 2024-09-24 附近是否发生断点式变化。Escape/Retest 是 transition 后的**结果标签**，不是研究对象本身；CLEAN 后连续 120 交易日不重入长底 = escape（`CLEAN_ESCAPE_HORIZON=120`）。
- **只消费 Lane 2 输入**：`outputs/research/etf_bottom/backtest_v1/v1_signal_daily.parquet`（trade_date/fund_code/fund_name/etf_type/industry_cluster/long_term_bottom/bottom_state/pos60/120/360，2022-01-04→2026-08-31，1289 ETF）+ `data/etf_signal/raw/*.parquet`（价格）。RAW 口径剔除 money/bond/commodity（近零波动，底部判定无异议）。不改/不读/不覆盖任何 Lane 1/2 逻辑。
- **Right-censor 口径（写死点）**：用**全市场交易日历**（`calendar.MarketCalendar`，取自 v1 的 trade_date 并集），不用单只 ETF 自身报价行数；停牌/缺失不拉长窗口，个体 OHLCV 完整度另记 `forward_data_complete_{h}d`。`escape_{h}d / right_censored_{h}d / days_to_first_retest` 是 trajectory 原始事实字段；ESCAPE/RAPID_RETEST/DELAYED 只是 `derive_report_labels` 的 report-only 派生（不互斥，不进 target）。
- **轨迹提取**（`trajectory.py`）：`build_persistence_series` 把 long_term_bottom 按 persistence window（RAW=0 / 3 / 5）转成连续确认序列（连续 N 交易日同向才翻转）；`extract_trajectories` 按每 on 段提取，核心 anchor = first_exit_date，字段含 entry_date/first_exit_date/first_retest_date/second_exit_date/observation_end/days_to_first_retest(is_current 后置 None)/days_in_bottom_total/n_retests/escape_*/right_censored_*/forward_data_complete_*。
- **生存分析**（`survival.py`，手写无 lifelines）：`survival_curve` = 每个 h 的 escape 生存比例（分母=有完整 h 市场交易日窗口的 exit，分子=到 h 仍无 retest；right_censored 不进入分母也绝不当作 escape）；`kaplan_meier` = 手写 KM（离散交易日，right-censored at h）。
- **断点检验**（`structural_break.py`）：`hypothesis_break` 按 break_date ± window(±40/±63/±90 交易日) 划分 pre/post 算 escape 比例与 effect(post−pre)；`data_driven_breakpoint` 扫描候选断点找 argmax；`date_block_bootstrap`（**重抽样单位=交易日块**，块=当天全部 exit，保留横截面相关）给 95% CI/p(效应>0)；`persistence_consistency` 遍历 {RAW,3,5} 三口径（**不同 persistence 用不同 trajectory 样本**，非对同表重复）。
- **统计语义分离（写死点）**：event-weighted estimate（每 exit 一票）只给点估计；date-block bootstrap inference 才做显著性。3A decision 是可执行 checklist **P1-P5，不做经济阈值**：P1 persistence 鲁棒性（3/5 同向）· P2 RAW/3D/5D 一致性 · P3 ±40/±63/±90 窗口效应同向 · P4 date-block 95% CI 不跨 0 · P5 市场控制（etf_type 分层效应保留）。
- **实测结论（2026-08-31 快照，PASS 5/5）**：escape 生存比例 20d 0.66 / 60d 0.48 / 120d 0.35 / 250d 0.25（大多数底部 exit 会在一个季度内重测）；**2024-09-24 ±63 效应 post−pre = +0.676（pre 0.295 → post 0.971）**，date-block CI [0.50, 0.80] 不跨 0（P4）、persistence 0.38/0.68/0.71 全同向（越持久效应越大，P1/P2）、±40/±63/±90 全正（P3）、全部 etf_type 同向（P5）；但**数据自身 argmax 断点在 2023-11-13（距 924 达 316 日）**——「底部切换后更持久」的结构跳变可能始于 2023 年末，924 更像随后的延续/强化而非唯一断点，报告明确标注此不确定。
- **产物**（`outputs/research/trend_transition/`）：`study3a_{primary}_{robust}.json`（结构化结果）+ `study3a_trajectories_{primary|raw|robust}.parquet`（含 escape/right_censored 原始事实）+ `study3a_report.html`（结论 + PASS checklist + 现象 + 断点检验 + 数据驱动断点 + 市场控制 + 口径）；date-stamped JSON，report 用 canonical `study3a_report.html`。
- **测试**（`tests/research/test_trend_transition.py`，13 例）：MarketCalendar 右截断边界（全市场交易日历、非单 ETF 行数）、trajectory escape/retest/right-censor/persistence 确认、survival 与 KM 单调/端点一致性、_window_split pre/post、hypothesis effect 方向、date-block determinism、persistence 用不同样本、_escape_series 剔除 current/censored。
- **Study 3B · Predicting Trend Transition（2026-09-02）**：模块 `src/research/trend_transition/`（`research trend-transition study3b` / `make trend-transition-3b`）——回答「在 first_exit_date 当天，仅用当时及以前可观测信息，能否提前识别未来 120 个交易日不会重新回到底部的 ETF」。`y = ESCAPE_120D`，训练样本仅 `right_censored_120d == False` 且 `forward_data_complete == True`；**不预测收益率、不生成 BUY**，3C（`TREND_TRANSITION_V1` 冻结）仅当 PASS gate 全过才开启。
  - **特征工程（`features.py`，无 look-ahead）**：`load_close_panel`（raw 价格宽面板，无 raw 的 fund 全 NaN 列安全降级）+ `FeatureBuilder`（per-fund as-of 序列，ffill 点值 / 窗口 nanmin）。**所有 feature_timestamp <= first_exit_date**，`max_date` 截断参数供测试验证「删 first_exit 后数据特征完全不变」。5 family：F1 position（pos60/120/360 + delta_*_5d/20d）· F2 rps（RPS20/60 当日横截面百分位 + delta）· F3 drawdown（max_dd_20/60 + previous/current depth + shallowing_ratio）· F4 bottom history（days_in_bottom/entry_to_exit/entry_pos/n_prior_bottom_episodes/exit_bottom_state）· F5 market（ltb breadth + delta、etf_market_return_20/60、etf_type_breadth + delta）。**禁止进模型**：year/month/calendar_date_numeric/post_924/days_since_924/y2025_dummy/y2026_dummy（`FORBIDDEN_FEATURES`，仅保留供诊断切片）。
  - **评估（`metrics.py`，手写无 sklearn）**：ROC-AUC（rank-based Mann-Whitney 平均秩）、PR-AUC（AP 积分）、Brier、calibration buckets、decision lift（Top 10/20/30%）。**双口径（§17 同日期评估）**：event-weighted（每 exit 一票）+ date-weighted（按 first_exit_date 聚合后 `roc_auc_soft` 软标签 AUC）——避免单日几十只 exit 支配结论。
  - **模型（`study3b_models.py`，手写）**：`Preprocessor`（连续中位填充+标准化、categorical one-hot，**只 fit train**）；M0 `LogisticRegressionL2`（IRLS Newton + L2 岭，截距不惩罚）；M1 `ShallowTree`（Gini CART，max_depth<=3 / min_samples_leaf 高，仅非线性稳健性）；M2 `SimpleScore`（方向已知稳定特征的 0/1/2 分箱打分，最终冻结 V1 优先冻结此类低参数量规则）。`score_feats` 由 `select_score_features` 从 discovery + regime 方向稳定性自动选，默认 individual-only（F1-F4，避免市场特征成为 regime 代理）。
  - **验证（`study3b_validation.py`）**：`make_trainable`（非右截断+forward 完整+确定标签）；`feature_discovery`（§9：每特征 escape/retest 中位、中位差、Cohen d、Spearman、五等分表）；`regime_robustness`（§10：年份 + pre-2023-11/mid/post-924 段，方向一致性只统计 `min_year_n>=100` 的年份，避免 n=41/3 的 2025/2026 误判）；`expanding_walkforward`（§11：train=year<val 年，**绝不 shuffle**，preprocess/calibration 只 fit train，全部 prediction 为 OOS，pooled + 逐 fold）；`compare_models`（§15：BaseRate/Market Only/Position Only/Individual Only/Full/Full logistic/Simple Score）；`ablation`（§16 逐 family 删除）；`pass_gate`（§19 B1-B5，阈值 `GATE` 常量）。
  - **PASS Gate B1-B5（阈值锁定）**：B1 Individual Increment（Full − Market Only pooled OOS AUC ≥ 0.05）· B2 OOS Direction（Full AUC ≥ 0.55 + top 特征方向跨 fold 一致）· B3 Decision Lift（Top20% lift ≥ 1.30 且 rate−base ≥ 0.10）· B4 Calibration（Brier < base-rate + bucket 单调 ≥ 0.50）· B5 Mechanism Stability（≥2 个 family ablation drop ≥ 0.01）。verdict：`PASS — PREDICTABLE_TRANSITION` / `FAIL — MARKET_REGIME_ONLY`（B1）· `FAIL — TIME_DEPENDENT`（B2）· `FAIL — NO_DECISION_LIFT`（B3/B4）· `FAIL — UNSTABLE_FEATURES`（B5）。
  - **实测结论（2026-08-31 快照，FAIL — MARKET_REGIME_ONLY 0/5）**：base rate 34.6%（2022=33.5%/2023=3.3%/2024=52.9%/2025=80.5%/2026=0%，高度 regime-dependent）；Individual Only pooled OOS AUC **0.56** > Market Only **0.40**，但 Full（含市场特征）= **0.41** ≈ Market——市场特征（breadth delta 等）跨 924 断点反号，加入后把个体信号拖回随机；Simple Score 0.80 event / 0.45 date（date 口径暴露市场代理成分）；2025/2026 OOS 极稀疏（n=41/3，§11 fold 设计固有）；discovery 最强为 Position velocity（delta_pos60/120_5d/20d，spearman≈0.32-0.34）与 RPS（rps60），但方向跨 regime 不稳；ablation 仅 F2_rps 有正增量。**结论：Lane 3 尚无独立 ETF-level selection value，不进入 3C**；这是 3B 设计预期的诚实 FAIL（防止把 924 regime 效应误当可交易信号）。
  - **产物**（`outputs/research/trend_transition/`）：`study3b_dataset.parquet`（事件级特征数据集）· `study3b_feature_analysis.csv`（§9 逐特征）· `study3b_walkforward.json`（逐 fold OOS）· `study3b_model_comparison.json`（§15）· `study3b_ablation.json`（§16）· `study3b_calibration.csv` · `study3b_predictions.parquet`（OOS event-level：fund_code/first_exit_date/y_true/y_prob/fold/model）· `study3b_summary.json`（汇总+gate+robustness，报告唯一事实源）· `study3b_report.html`（首页回答 6 问：base rate / Market Only 判别力 / Individual 增量 / Top20% lift / 2025-2026 OOS / 2-3 个 mechanism，verdict 收尾；纯 renderer 只读 summary.json + walkforward.json，不重算）。
  - **Robustness R1-R4**（`study3b.py _robustness`）：R1 persistence（RAW/3D/5D 独立 trajectory 样本方向一致）· R2 horizon（ESCAPE_60D/250D 主模型 AUC）· R3 time OOS（2025/2026 单独展示）· R4 etf_type（industry/theme/broad 分组 AUC）。
  - **CLI**：`python src/main.py research trend-transition study3b [--persistence 0|3|5 --horizon 60|120|250 --l2 1.0]`；`make trend-transition-3b`。只读缓存不联网，无副作用（只写 `outputs/research/trend_transition/`）。
  - **测试**（`tests/research/test_trend_transition_3b.py`，20 例，§25 锁定）：as-of 截断不变性（特征 ≤ first_exit）、删 first_exit 后数据特征完全不变、right_censored 不进训练集、outcome 字段不进模型矩阵、train 早于 val、walk-forward 不 shuffle 且确定性、preprocess 只 fit train（中位/均/std）、calibration 只 fit train（SimpleScore 边界）、predictions 全 OOS、无 random split、persistence 用独立样本、renderer 纯消费 JSON、metrics 正确性（AUC/PR-AUC/soft-AUC/lift/date-weighted）、discovery 五等分单调、regime 一致性容忍小年份、score 选 individual-first、logistic 可分离+系数方向、tree 深度受限。
- **Study 3C · Trend Transition State Classification（2026-09-02）**：模块 `src/research/trend_transition/`（`research trend-transition study3c` / `make trend-transition-3c`）——**不再预测未来、不生成 BUY**。唯一任务：在任意 as-of date 用当日及以前数据为每只可靠 ETF 标记其 Trend Transition 生命周期状态，并由全市场状态分布识别市场是否正在从底部向趋势迁移。**STATE != SIGNAL / PREDICTION / BUY**。PASS gate C1-C5 全过 → 冻结 `config/research/trend_transition_state_v1.yaml`（`TREND_TRANSITION_STATE_V1`，`FROZEN_STATE_CLASSIFIER`）。
  - **状态机（`state_classifier.py`，确定性 as-of）**：8 状态 `UNRELIABLE / BOTTOM / FIRST_EXIT / TRANSITION_EARLY / TRANSITION_ACTIVE / TRANSITION_ESTABLISHED / RETEST / POST_TRANSITION`（LEADERSHIP 属 Lane 1，不进 Lane 3）。`confirmed_long_term_bottom` = `build_persistence_series(long_term_bottom, window=3)`（向后滚动确认，天然无 look-ahead）。`first_exit_date` = confirmed 首次 True→False 翻转当日（day 0，`days_since_first_exit` 用全市场日历位置差）。age 窗口：FIRST_EXIT 0-5 · EARLY 6-20 · ACTIVE 21-60 · ESTABLISHED 61-120 · POST >120。**left-censored**（窗口首日即 non-bottom、无法观测 first_exit）→ POST_TRANSITION + `state_origin=LEFT_CENSORED`、`transition_anchor_known=False`、不伪造 first_exit（§15）；此类基金重入底部判 RETEST 而非 BOTTOM。UNRELIABLE 优先级最高（§6，不进 breadth 分母），状态变量跨不可靠段保留。事件检测只在 reliable 日进行。**不用 RPS/drawdown/breadth/etf_type 决定主状态**（§16，那些只进 state_context）。
  - **市场指标（`state_metrics.py`，§19-§25）**：`bottom_breadth=(BOTTOM+RETEST)/reliable` · `transition_breadth=(FIRST_EXIT..ESTABLISHED)/reliable` · `active_transition_breadth=(FIRST_EXIT+EARLY+ACTIVE)/reliable`（**Lane 3 核心市场指标**）· Transition Flow（new_first_exit/retest/active/established/post + `net_transition_flow=new_first_exit−new_retest`）· Momentum（breadth 5d/20d 变化）· ETF Type 分层（§23 每日各 etf_type）· Lane 2 联动（BOTTOM/RETEST × target_stage，只读）· **Lane 1 联动（辅助验证）**：读 `watchlist_{date}.parquet` 的 `trend_state`（仅近期存在），**覆盖率不影响 3C PASS/FAIL**（用户锁定）。
  - **验证 C1-C5（`study3c.py`，§27-§28，不做经济阈值）**：C1 Determinism（重跑一致）· C2 No look-ahead（截断未来 → 状态不变，抽查 6 日）· C3 Legal transitions（`LEGAL_TRANSITIONS` 含跨可靠段空隙的合法 age 推进，illegal=0）· C4 Persistence robust（RAW/3D/5D breadth 相关性 ≥0.7，实测 0.97/0.97）· C5 Historical interpretability（2024 transition 强度 > 2022-23 基线 ∧ post-924 retest 率 < pre-2024 一半）。verdict：`PASS — TREND_TRANSITION_STATE_V1_READY` 或对应 FAIL。
  - **实测结论（2026-08-31 快照，PASS 5/5）**：2022-23 高 retest（pre-2024 日均 retest 2.68）/低 persistence；2024 transition_breadth 升至 0.41（Q4 post-924 达 0.54）；post-924 retest 率降至 0.74/日（escape 强化，复现 3A）；2025 市场成熟（多数 POST_TRANSITION，active breadth 回落）→ 2026Q2-3 新一轮 transition（active 2%→14%）。当前 1186 只 reliable：POST 948 / ACTIVE 168 / EARLY 25 / FIRST_EXIT 12 / RETEST 33，Active Transition Breadth 17.3%（20D +2.6pp）。
  - **产物**（`outputs/research/trend_transition/`）：`study3c_state_history.parquet`（逐日×逐 ETF 状态历史，含全部 §17 字段）· `study3c_market_history.parquet`（每日 breadth/flow/momentum）· `study3c_current_state.parquet`（as-of 最新日）· `study3c_transition_flows.csv` · `study3c_etf_type_breakdown.csv` · `study3c_validation.json`（C1-C5）· `study3c_summary.json`（报告唯一事实源）· `study3c_report.html`（KPI + Active Transition Breadth + 一句话结论 §38 + State Table §39 + 历史 Map SVG + 验证）。PASS 后冻结 `config/research/trend_transition_state_v1.yaml`。
  - **Application（`research trend-transition state --date YYYYMMDD` / `make trend-transition-state DATE=...`）**：读冻结 YAML（未冻结/状态非 FROZEN → 拒绝运行），输出 date-stamped `trend_transition_state_{date}.{parquet,json}`，**不写 `_latest`**。
  - **测试**（`tests/research/test_trend_transition_3c.py`，16 例，§41 锁定）：same input→same state、删未来数据状态不变、3D 确认 first exit/retest、age 五区间、retest 优先于 age、UNRELIABLE 最高优先、illegal=0、left-censor 不伪造 first_exit、breadth 分母仅 reliable、flow=真实当日转换、etf_type reconcile、Lane overlap 只读、RAW/3D/5D 独立、renderer 纯消费 JSON、frozen YAML 后 Application 只读规则。
