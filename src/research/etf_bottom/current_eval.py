...
from src.common.paths import etf_signal_raw_dir, selection_universe_path
...
AS_OF = "2026-08-28"
POS_LOW_360 = 20.0   # long_term_bottom 定义：pos120<=20 且 pos360<=20
CORP_ACTION_RET = 0.20


def load_watch_etfs() -> pd.DataFrame:
    """从 selection_universe.yaml 读取主题 ETF 资产（theme_etf + sub_industry_etf）。"""
    import yaml
    ...
    for theme_key, theme in cfg["themes"].items():
        for tier in theme["tiers"]:
            if tier["key"] not in ("theme_etf", "sub_industry_etf"):
                continue
...
def load_frozen_cutpoints() -> tuple[list[float], list[float]]:
    """从 repair_structure.json 原样读取 2E 冻结切分点。"""
    rs = json.loads((STUDY_DIR / "repair_structure.json").read_text(encoding="utf-8"))
    cp = rs["discovery_universe"]["cut_points"]
    return cp["price_pos_120"], cp["price_pos_60"]
...
def eval_one(code: str, name: str, theme: str, cut120: list[float], cut60: list[float]) -> dict[str, Any]:
    ...
    if not full_360 or unreliable_360:
        return {
            "fund_code": code, "fund_name": name, "theme": theme,
    ...
    if not (p120 is not None and p360 is not None and p120 <= POS_LOW_360 and p360 <= POS_LOW_360):
        return {
            "fund_code": code, "fund_name": name, "theme": theme,
...
def run_current_eval(study_dir: Path | None = None) -> dict:
    ...
    payload = {
        ...
        "as_of": AS_OF,
        "cut_points_source": "repair_structure.json (frozen 2E, read as-is)",
